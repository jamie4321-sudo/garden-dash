...
function saveSettlement_(arr) {
  ...
  try {
    ...
    sh.clear();
    var head = ['date', 'place', 'category', 'title', 'vendor', 'amount', 'status', 'paidDate', 'statementUrl', 'photoUrl', 'memo'];
    var rows = [head];
    (arr || []).forEach(function (x) {
      rows.push([x.date || '', x.place || '', x.category || '', x.title || '', x.vendor || '',
        (x.amount == null ? '' : x.amount), x.status || '', x.paidDate || '', x.statementUrl || '', x.photoUrl || '', x.memo || '']);
    });
    sh.getRange(1, 1, sh.getMaxRows(), head.length).setNumberFormat('@');
    sh.getRange(1, 1, rows.length, head.length).setValues(rows);
    sh.setFrozenRows(1);
