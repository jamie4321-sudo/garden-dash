...
## ✍️ 화면 편집 → 시트 저장(쓰기) 활성화

주간 스케줄을 화면에서 편집하면 구글 시트에도 자동 저장됩니다(`doPost`).

1. `js/config.js` 에서 `WRITE_BACK: true` 확인 (기본값 true) + `API_URL` 설정
2. **Code.gs 를 수정했으므로 재배포 필요**:
   - Apps Script → **배포 > 배포 관리** → 기존 웹 앱 배포의 ✏️(수정) → **버전: 새 버전** → **배포**
   - (URL 은 그대로 유지됩니다. 새 배포를 만들면 URL 이 바뀌니 "배포 관리"에서 버전만 올리세요.)
3. 편집 시 화면 우하단에 **"시트에 저장됨 ✓"** 토스트가 뜨면 성공.

> ⚠️ **보안 주의**: 웹 앱 액세스를 "모든 사용자"로 두면 URL 을 아는 누구나 시트에 쓸 수 있습니다.
> 내부용/데모라면 괜찮지만, 외부 공개 시에는 (a) 액세스 권한을 "본인/도메인 내"로 제한하거나
> (b) 공유 토큰 검증을 추가하는 것을 권장합니다. 필요하면 토큰 방식으로 확장해 드릴 수 있습니다.
...
층별 현장 사진을 구글 드라이브에서 불러와 대시보드에 갤러리로 표시합니다.

- **드라이브 폴더**: [GARDEN 각층 현황](https://drive.google.com/drive/folders/1JF5VTpU-ldB2jbIZlQUBlPXYof56Bp1s) (부모 폴더, `Code.gs`의 `FLOOR_FOLDER_ID`에 연결됨)
- 부모 폴더 아래 **하위폴더(층) 자동 인식** → 담당자가 폴더에 사진만 올리면 대시보드에 반영
- `doGet?action=floors` 가 층별 사진 목록(썸네일·원본 링크)을 JSON으로 반환

### ⚠️ 공유 설정 필수
대시보드(공개 사이트)에서 사진 썸네일이 보이려면 **부모 폴더를 "링크가 있는 모든 사용자 - 뷰어"로 공유**해야 합니다.
1. 드라이브에서 **GARDEN 각층 현황** 폴더 우클릭 → **공유**
2. "일반 액세스" → **링크가 있는 모든 사용자** → 역할 **뷰어**
3. 하위폴더·사진은 부모 설정을 상속하므로 한 번만 하면 됩니다.
